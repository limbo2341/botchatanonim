...
async def a_stats_cb(call: types.CallbackQuery):
    ...
    prem_row  = await db_exec("SELECT COUNT(*) FROM users WHERE prem_until > $1", (now,), "one")
    ban_row   = await db_exec("SELECT COUNT(*) FROM users WHERE banned_until > $1", (now,), "one")
    mute_row  = await db_exec("SELECT COUNT(*) FROM users WHERE muted_until > $1", (now,), "one")
    warn_row  = await db_exec("SELECT COUNT(*) FROM users WHERE warns > 0", fetch="one")
    rep_row   = await db_exec("SELECT COUNT(*) FROM reports", fetch="one")
    ch_row    = await db_exec("SELECT COUNT(*) FROM ad_channels", fetch="one")
    prem_c  = prem_row[0]  if prem_row  else 0
    ban_c   = ban_row[0]   if ban_row   else 0
    mute_c  = mute_row[0]  if mute_row  else 0
    warn_c  = warn_row[0]  if warn_row  else 0
    rep_c   = rep_row[0]   if rep_row   else 0
    ch_c    = ch_row[0]    if ch_row    else 0
...
async def global_handler(message: types.Message, state: FSMContext):
    ...
    # ---- ГЛАВНОЕ МЕНЮ ----
        ...
        u = await db_exec("SELECT * FROM users WHERE id=$1", (uid,), "one")
        if not u:
