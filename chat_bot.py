...
# chat_id -> message_id для авто-обновления
live_stats_messages   = {}   # {chat_id: message_id}
live_banlist_messages = {}   # {chat_id: message_id}
queues = {
    "all": [], "М": [], "Ж": [], "Music": [], "Games": [],
...
async def menu_profile(message: types.Message, state: FSMContext):
    uid = message.from_user.id
    u = await db_exec(
        "SELECT id,username,refs,prem_until,gender,age,warns,banned_until FROM users WHERE id=$1",
        (uid,), "one"
    )
    if not u:
    ...
    if uid in ADMINS:
        rank = "👑 Администратор"
    elif u['prem_until'] and u['prem_until'] > now:
        rank = "💎 Premium"
    else:
        rank = "👤 Пользователь"

    # Premium
    if u['prem_until'] and u['prem_until'] > now:
        prem_line = "✅ Активен (до " + u['prem_until'].strftime('%d.%m.%Y') + ")"
    else:
        prem_line = "❌ Не активен"

    # Пол
    gender_line = {"М": "М", "Ж": "Ж"}.get(u['gender'] or "", "—")
    ...
    text = (
        ...
        "💎 Premium: " + prem_line + "\n"
        "🤝 Рефералов: " + str(refs) + "\n"
        "🚻 Пол: " + gender_line + " | 🎂 Возраст: " + age + "\n"
        "⚠️ Варнов: " + str(warns) + "/3\n"
        "➖➖➖➖➖➖➖➖➖➖"
    )

    kb = InlineKeyboardBuilder()
    kb.button(text="✏️ Редактировать профиль", callback_data="edit_profile")
    await message.answer(text, reply_markup=kb.adjust(1).as_markup(), parse_mode="Markdown")
...
async def a_stats_cb(call: types.CallbackQuery):
    if call.from_user.id not in ADMINS:
        ...
        return
    now = datetime.now()
    r1 = await db_exec("SELECT COUNT(*) AS cnt FROM users", (), "one")
    r2 = await db_exec("SELECT COUNT(*) AS cnt FROM users WHERE prem_until > $1", (now,), "one")
    r3 = await db_exec("SELECT COUNT(*) AS cnt FROM users WHERE banned_until > $1", (now,), "one")
    r4 = await db_exec("SELECT COUNT(*) AS cnt FROM users WHERE muted_until > $1", (now,), "one")
    r5 = await db_exec("SELECT COUNT(*) AS cnt FROM users WHERE warns > 0", (), "one")
    r6 = await db_exec("SELECT COUNT(*) AS cnt FROM reports", (), "one")
    r7 = await db_exec("SELECT COUNT(*) AS cnt FROM ad_channels", (), "one")
    total_c = r1['cnt'] if r1 else 0
    prem_c  = r2['cnt'] if r2 else 0
    ban_c   = r3['cnt'] if r3 else 0
    mute_c  = r4['cnt'] if r4 else 0
    warn_c  = r5['cnt'] if r5 else 0
    rep_c   = r6['cnt'] if r6 else 0
    ch_c    = r7['cnt'] if r7 else 0
    online  = len(active_chats) // 2
    lines = [
        "📊 *Статистика бота*\n",
        "👥 Всего юзеров: " + str(total_c),
        "💎 Premium: " + str(prem_c),
        "🚫 Забаненных: " + str(ban_c),
        "🔇 Замученных: " + str(mute_c),
        "⚠️ С варнами: " + str(warn_c),
        "🚩 Жалоб в очереди: " + str(rep_c),
        "💬 Активных чатов: " + str(online),
        "📣 Рекл. каналов: " + str(ch_c),
    ]
    txt = "\n".join(lines)
    kb = InlineKeyboardBuilder()
    kb.button(text="🔄 Обновить", callback_data="a_stats_refresh")
    sent = await call.message.answer(txt, reply_markup=kb.adjust(1).as_markup(), parse_mode="Markdown")
    live_stats_messages[call.message.chat.id] = sent.message_id
    await call.answer()

@dp.callback_query(F.data == "a_stats_refresh")
async def a_stats_refresh_cb(call: types.CallbackQuery):
    if call.from_user.id not in ADMINS:
        await call.answer()
        return
    now = datetime.now()
    ...
    try:
        ...
        live_stats_messages[call.message.chat.id] = call.message.message_id
    except Exception:
        pass
...
async def adm_banlist(call: types.CallbackQuery):
    ...
    ban_c  = len(banned)  if banned  else 0
    mute_c = len(muted)   if muted   else 0
    warn_c = len(warned)  if warned  else 0
    lines = ["📋 *Активные наказания*", "🚫 Банов: " + str(ban_c) + " | 🔇 Мутов: " + str(mute_c) + " | ⚠️ С варнами: " + str(warn_c), ""]
    lines.append("🔨 *Забаненные:*")
    if banned:
        for r in banned:
            lines.append("• `" + str(r['id']) + "` @" + (r['username'] or "—") + " до " + r['banned_until'].strftime('%d.%m.%Y'))
    else:
        lines.append("Нет")
    lines.append("")
    lines.append("🔇 *Замученные:*")
    if muted:
        for r in muted:
            lines.append("• `" + str(r['id']) + "` @" + (r['username'] or "—") + " до " + r['muted_until'].strftime('%d.%m.%Y'))
    else:
        lines.append("Нет")
    lines.append("")
    lines.append("⚠️ *С варнами:*")
    if warned:
        for r in warned:
            lines.append("• `" + str(r['id']) + "` @" + (r['username'] or "—") + " — " + str(r['warns']) + "/3")
    else:
        lines.append("Нет")
    txt = "\n".join(lines)
    kb = InlineKeyboardBuilder()
    kb.button(text="🔄 Обновить", callback_data="adm_banlist_refresh")
    sent = await call.message.answer(txt, reply_markup=kb.adjust(1).as_markup(), parse_mode="Markdown")
    live_banlist_messages[call.message.chat.id] = sent.message_id
    await call.answer()

@dp.callback_query(F.data == "adm_banlist_refresh")
async def adm_banlist_refresh(call: types.CallbackQuery):
    if call.from_user.id not in ADMINS:
        await call.answer()
        return
    now = datetime.now()
    ...
    try:
        await call.message.edit_text(txt, reply_markup=kb.adjust(1).as_markup(), parse_mode="Markdown")
        live_banlist_messages[call.message.chat.id] = call.message.message_id
    except Exception:
        pass
    await call.answer("✅ Обновлено!")
...
# ================================================================
# АВТО-ОБНОВЛЕНИЕ статистики и банлиста каждые 60 секунд
# ================================================================
async def auto_update_loop():
    ...
    while True:
        ...
        if live_stats_messages:
            ...
            lines = [
                "📊 *Статистика бота*\n",
                "👥 Всего юзеров: " + str(r1['cnt'] if r1 else 0),
                "💎 Premium: " + str(r2['cnt'] if r2 else 0),
                "🚫 Забаненных: " + str(r3['cnt'] if r3 else 0),
                "🔇 Замученных: " + str(r4['cnt'] if r4 else 0),
                "⚠️ С варнами: " + str(r5['cnt'] if r5 else 0),
                "🚩 Жалоб в очереди: " + str(r6['cnt'] if r6 else 0),
                "💬 Активных чатов: " + str(len(active_chats) // 2),
                "📣 Рекл. каналов: " + str(r7['cnt'] if r7 else 0),
            ]
            txt = "\n".join(lines)
            kb = InlineKeyboardBuilder()
            kb.button(text="🔄 Обновить", callback_data="a_stats_refresh")
            ...
            dead = []
            for chat_id, msg_id in list(live_stats_messages.items()):
                try:
                    ...
                    dead.append(chat_id)
            for chat_id in dead:
                live_stats_messages.pop(chat_id, None)

        # --- Банлист ---
        if live_banlist_messages:
            now = datetime.now()
            banned = await db_exec(
                "SELECT id,username,banned_until FROM users WHERE banned_until > $1 ORDER BY banned_until DESC LIMIT 10",
                (now,), "all"
            ...
            mute_c = len(muted)   if muted   else 0
            warn_c = len(warned)  if warned  else 0
            lines = ["📋 *Активные наказания*",
                     "🚫 Банов: " + str(ban_c) + " | 🔇 Мутов: " + str(mute_c) + " | ⚠️ С варнами: " + str(warn_c), ""]
            lines.append("🔨 *Забаненные:*")
            if banned:
                for r in banned:
                    lines.append("• `" + str(r['id']) + "` @" + (r['username'] or "—") + " до " + r['banned_until'].strftime('%d.%m.%Y'))
            ...
            markup = kb.adjust(1).as_markup()
            dead = []
            for chat_id, msg_id in list(live_banlist_messages.items()):
                try:
                    await bot.edit_message_text(txt, chat_id=chat_id, message_id=msg_id,
                                                reply_markup=markup, parse_mode="Markdown")
                except Exception:
                    dead.append(chat_id)
            for chat_id in dead:
                live_banlist_messages.pop(chat_id, None)
