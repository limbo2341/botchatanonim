...
def init_db():
    ...
    try:
        db_exec("ALTER TABLE users ADD COLUMN warns INTEGER DEFAULT 0")
    except:
        pass
...
def get_warns(uid):
    r = db_exec("SELECT warns FROM users WHERE id=?", (uid,), "one")
    return r[0] if r and r[0] else 0

def get_rank(uid):
    if uid in ADMINS: return "👑 Администратор"
    if is_prem(uid): return "💎 Premium"
    return "👤 Пользователь"
...
async def enter_queue(uid, cat):
    ...
    if lst:
        ...
        # Для обычных пользователей — базовая карточка без варнов/ранга
        uid_card_basic = uid_card if is_prem(uid) else _basic_card(pid)
        pid_card_basic = pid_card if is_prem(pid) else _basic_card(uid)
...
def _basic_card(uid):
    ...
    r = db_exec("SELECT username, gender, age FROM users WHERE id=?", (uid,), "one")
    if not r:
        return f"ID: `{uid}`\nИнфо не найдено"
...
async def chat_report(message: types.Message, state: FSMContext):
    ...
    db_exec("INSERT INTO reports (reporter_id, target_id, last_message) VALUES (?,?,?)", (uid, pid, last_msg))
...
async def start(message: types.Message, state: FSMContext):
    ...
    if is_new:
        db_exec("INSERT INTO users (id, username, refs) VALUES (?,?,0)", (uid, message.from_user.username))
        if ref_id:
            try:
                rid = int(ref_id)
...
async def ep_gender_set(call: types.CallbackQuery):
    gender = call.data[4:]
    db_exec("UPDATE users SET gender=? WHERE id=?", (gender, call.from_user.id))
...
async def ep_age_set(call: types.CallbackQuery):
    age = call.data[4:]
    db_exec("UPDATE users SET age=? WHERE id=?", (age, call.from_user.id))
...
async def adm_reps_cb(call: types.CallbackQuery):
    r = db_exec("SELECT id,reporter_id,target_id,last_message,created_at FROM reports ORDER BY id ASC LIMIT 1", fetch="one")
    if not r:
...
async def r_ban_cb(call: types.CallbackQuery):
    ...
    if rep_id > 0: db_exec("DELETE FROM reports WHERE id=?", (rep_id,))
...
async def r_warn_cb(call: types.CallbackQuery):
    ...
    db_exec("UPDATE users SET warns=COALESCE(warns,0)+1 WHERE id=?", (target_id,))
    warns = get_warns(target_id)

    if warns >= 3:
        until = (datetime.now() + timedelta(days=3)).isoformat()
        db_exec("UPDATE users SET banned_until=?, warns=0 WHERE id=?", (until, target_id))
        if target_id in active_chats:
            pid = active_chats.pop(target_id)
            active_chats.pop(pid, None)
            try: await bot.send_message(pid, "💔 Собеседник покинул чат.", reply_markup=main_kb(pid))
...
async def adm_warn_proc(message: types.Message, state: FSMContext):
    ...
    db_exec("UPDATE users SET warns=COALESCE(warns,0)+1 WHERE id=?", (tid,))
    warns = get_warns(tid)
    name = f"@{tusername}" if tusername else str(tid)

    if warns >= 3:
        until = (datetime.now() + timedelta(days=3)).isoformat()
        db_exec("UPDATE users SET banned_until=?, warns=0 WHERE id=?", (until, tid))
...
async def adm_unwarn_proc(message: types.Message, state: FSMContext):
    ...
    try:
        tid = int(raw)
        row = db_exec("SELECT id,username,warns FROM users WHERE id=?", (tid,), "one")
    except ValueError:
        row = db_exec("SELECT id,username,warns FROM users WHERE username=?", (raw,), "one")
    ...
    new_warns = warns - 1
    db_exec("UPDATE users SET warns=? WHERE id=?", (new_warns, tid))
    try:
...
async def adm_banlist(call: types.CallbackQuery):
    ...
    banned = db_exec("SELECT id,username,banned_until FROM users WHERE banned_until > ? ORDER BY banned_until DESC LIMIT 10", (now,), "all")
    muted  = db_exec("SELECT id,username,muted_until FROM users WHERE muted_until > ? ORDER BY muted_until DESC LIMIT 10", (now,), "all")
    warned = db_exec("SELECT id,username,warns FROM users WHERE warns > 0 ORDER BY warns DESC LIMIT 10", fetch="all")

    txt = "📋 *Активные наказания*\n\n🔨 *Забаненные:*\n"
    if banned:
        for r in banned:
            t = datetime.fromisoformat(r[2]).strftime('%d.%m.%Y')
...
async def adm_lookup_proc(message: types.Message, state: FSMContext):
    ...
    try:
        tid = int(raw)
        row = db_exec("SELECT id,username,refs,prem_until,gender,age,banned_until,muted_until,warns FROM users WHERE id=?", (tid,), "one")
    except ValueError:
        row = db_exec("SELECT id,username,refs,prem_until,gender,age,banned_until,muted_until,warns FROM users WHERE username=?", (raw,), "one")
    ...
    if prem_until:
        try:
            p = datetime.fromisoformat(prem_until)
            prem_str = f"✅ до {p.strftime('%d.%m.%Y')}" if p > now else "❌ Истёк"
        except: pass

    ban_str = "—"
    if banned_until:
        try:
            b = datetime.fromisoformat(banned_until)
            ban_str = f"🔨 до {b.strftime('%d.%m.%Y %H:%M')}" if b > now else "—"
        except: pass

    mute_str = "—"
    if muted_until:
        try:
            m = datetime.fromisoformat(muted_until)
            mute_str = f"🔇 до {m.strftime('%d.%m.%Y %H:%M')}" if m > now else "—"
        except: pass
...
async def quick_action_cb(call: types.CallbackQuery):
    ...
    if action == "warn":
        ...
        db_exec("UPDATE users SET warns=warns-1 WHERE id=?", (tid,))
        new = warns - 1
...
async def adm_reset_warns_proc(message: types.Message, state: FSMContext):
    ...
    warns = warns or 0
    name = f"@{tusername}" if tusername else str(tid)
    db_exec("UPDATE users SET warns=0 WHERE id=?", (tid,))
    try:
...
async def adm_revoke_prem_proc(message: types.Message, state: FSMContext):
    ...
    try:
        tid = int(raw)
        row = db_exec("SELECT id,username,prem_until FROM users WHERE id=?", (tid,), "one")
    except ValueError:
        row = db_exec("SELECT id,username,prem_until FROM users WHERE username=?", (raw,), "one")
    ...
    if not prem_until or datetime.fromisoformat(prem_until) <= datetime.now():
        await message.answer(f"ℹ️ У {name} нет активного Premium.")
        await state.clear(); return

    db_exec("UPDATE users SET prem_until=NULL WHERE id=?", (tid,))
    try:
...
async def adm_mute_input(message: types.Message, state: FSMContext):
    ...
    if days == 999:
        ...
        until = (datetime.now() + timedelta(days=days)).isoformat()
        label = f"на {days} дн."
        msg = f"🔇 Вы замучены *на {days} дн.*\nОкончание: {datetime.fromisoformat(until).strftime('%d.%m.%Y %H:%M')}"
...
async def a_stats_cb(call: types.CallbackQuery):
    ...
    warn_c = db_exec("SELECT COUNT(*) FROM users WHERE warns > 0",        fetch="one")[0]
...
async def q_callback(call: types.CallbackQuery, state: FSMContext):
    ...
    # VIP только для премиум
    if cat == "VIP" and not is_prem(uid):
...
async def pay_adm(call: types.CallbackQuery):
    ...
    if action == "yes":
        if days == 999:
            ...
            msg = f"💎 *Premium на {days} дней!*\nДо: {datetime.fromisoformat(until).strftime('%d.%m.%Y')}"
