...
async def build_stats_text():
    ...
    total_c = cnt(await db_exec("SELECT COUNT(*) AS cnt FROM users", (), "one"))
    ...
    warn_c  = cnt(await db_exec("SELECT COUNT(*) AS cnt FROM users WHERE warns > 0", (), "one"))
    rep_c   = cnt(await db_exec("SELECT COUNT(*) AS cnt FROM reports", (), "one"))
    ch_c    = cnt(await db_exec("SELECT COUNT(*) AS cnt FROM ad_channels", (), "one"))
    online  = len(active_chats) // 2

    return (
        f"📊 *Статистика бота*\n\n"
        f"👥 Всего юзеров: {total_c}\n"
        f"💎 Premium: {prem_c}\n"
        f"🚫 Забаненных: {ban_c}\n"
        f"🔇 Замученных: {mute_c}\n"
...
async def build_banlist_text():
    ...
    txt = f"📋 *Активные наказания*\n🚫 Банов: {ban_c} | 🔇 Мутов: {mute_c} | ⚠️ С варнами: {warn_c}\n\n"
...
async def auto_update_loop():
    """Фоновый таск: обновляет живые сообщения статистики и банлиста каждые 60 сек."""
