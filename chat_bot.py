...
class States(StatesGroup):
    ...
    # Новые состояния для рекламных каналов
    add_ad_channel       = State()
    delete_ad_channel    = State()
...
async def get_user_info(uid):
    r = await db_exec("SELECT username, gender, age, prem_until FROM users WHERE id=$1", (uid,), "one")
    if not r:
        return f"ID: `{uid}`\nИнфо не найдено"
    username = r['username']
    gender = r['gender']
...
async def check_user_subscribed(uid, channel_url):
    """Проверяет подписку пользователя на канал/группу."""
    try:
        ...
        return True  # Если проверить нельзя — пропускаем
...
async def check_all_subscriptions(uid):
    ...
    not_subscribed = []
    for ch in channels:
        subscribed = await check_user_subscribed(uid, ch['url'])
        if not subscribed:
...
async def _basic_card(uid):
    """Базовая карточка собеседника для обычных юзеров."""
...
async def enter_queue(uid, cat):
    ...
    if lst:
        ...
        uid_info = await get_user_info(pid)
        pid_info = await get_user_info(uid)

        uid_card_basic = uid_info if await is_prem(uid) else await _basic_card(pid)
        pid_card_basic = pid_info if await is_prem(pid) else await _basic_card(uid)
...
async def chat_exit(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    # Если в активном чате — выходим из чата
    if uid in active_chats:
        ...
        return
    # Если в очереди поиска — убираем из очереди
    removed = False
    for k in queues:
        if uid in queues[k]:
            queues[k].remove(uid)
    ...
    if removed:
        ...
        await message.answer("Вы не в чате и не в поиске.", reply_markup=main_kb(uid))

@dp.message(StateFilter("*"), F.text == "➡️ Следующий")
async def chat_next(message: types.Message, state: FSMContext):
    uid = message.from_user.id
    await state.clear()
    # Если в активном чате — скипаем и ищем следующего
    if uid in active_chats:
        pid = active_chats.pop(uid)
        active_chats.pop(pid, None)
        last_messages.pop(uid, None)
        ...
        return
    # Если уже в очереди — сообщаем
    for k in queues:
        if uid in queues[k]:
            await message.answer("⏳ Вы уже в поиске... Ожидайте или нажмите ❌ Выйти для отмены.")
            return
    # Если нигде нет — просто ищем
    await enter_queue(uid, "all")
...
async def chat_report(message: types.Message, state: FSMContext):
    ...
    rep_id = rep_row['id'] if rep_row else 0
...
async def start(message: types.Message, state: FSMContext):
    ...
    if is_new:
        ...
        if ref_id:
            try:
                rid = int(ref_id)
                ref_exists = await db_exec("SELECT id FROM users WHERE id=$1", (rid,), "one")
                if rid != uid and ref_exists:
                    await db_exec("UPDATE users SET refs=refs+1 WHERE id=$1", (rid,))
                    u_refs_row = await db_exec("SELECT refs FROM users WHERE id=$1", (rid,), "one")
                    u_refs = u_refs_row['refs'] if u_refs_row else 0
                    try:
                    ...
                    if u_refs % 10 == 0:
                        cur = await db_exec("SELECT prem_until FROM users WHERE id=$1", (rid,), "one")
                        base = datetime.now()
                        if cur and cur['prem_until']:
                            try:
                                ex = cur['prem_until']
                                if ex > base: base = ex
                            except: pass
                        until = base + timedelta(days=2)
                        await db_exec("UPDATE users SET prem_until=$1 WHERE id=$2", (until, rid))
                        try:
                            await bot.send_message(rid, f"🏆 Вы пригласили *{u_refs}* человек — получаете *+2 дня Premium*!", parse_mode="Markdown")
                        except: pass
            except (ValueError, TypeError): pass

    # Проверяем рекламные каналы для новых пользователей
    channels = await get_ad_channels()
    if channels and is_new:
        await state.update_data(reg_age=None, reg_gender=None, reg_ref=ref_id)
        kb = InlineKeyboardBuilder()
        for ch in channels:
            title = ch['title'] or ch['url']
            kb.button(text=f"👉 {title}", url=ch['url'])
        kb.button(text="✅ Я подписался!", callback_data="check_sub_reg")
        txt = "📢 *Для завершения регистрации подпишись на каналы:*\n\n"
        for ch in channels:
            title = ch['title'] or ch['url']
            txt += f"• {title}: {ch['url']}\n"
    ...
    # Если каналов нет или пользователь уже зарегистрирован — сразу к возрасту
    kb = InlineKeyboardBuilder()
...
async def check_sub_reg(call: types.CallbackQuery, state: FSMContext):
    ...
    if not_sub:
        ...
        txt = "❌ *Не хватает подписки на:*\n\n"
        for ch in not_sub:
            title = ch['title'] or ch['url']
            txt += f"• {title}: {ch['url']}\n"
        txt += "\nПодпишись и нажми ✅"
        await call.message.edit_text(txt, reply_markup=kb.adjust(1).as_markup(), parse_mode="Markdown")
        return
    # Подписан на всё — продолжаем регистрацию
    kb = InlineKeyboardBuilder()
    for a in ["< 18", "18+", "25+", "65+"]:
        kb.button(text=a, callback_data=f"set_age_{a}")
    await call.message.edit_text("✅ Отлично! Теперь укажи свой возраст:", reply_markup=kb.adjust(2).as_markup())
    await call.answer()
...
async def adm_ad_channels(call: types.CallbackQuery):
    ...
    await call.message.answer(
        ...
        "Каналы которые добавлены сюда — пользователи обязаны на них подписаться при регистрации.\n\n"
        "Если каналов нет — бот не требует подписки.",
...
async def adm_ad_add_start(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        ...
        "Пришли ссылку на канал или группу.\n"
        "Например: `https://t.me/mychannel`\n\n"
        "❗ Бот должен быть администратором в этом канале/группе чтобы проверять подписку.",
        parse_mode="Markdown"
    )
    await call.answer()
...
async def adm_ad_add_proc(message: types.Message, state: FSMContext):
    ...
    if not url.startswith("http") and not url.startswith("@"):
        await message.answer("❌ Неверный формат. Пришли ссылку типа https://t.me/channel или @channel")
        await state.clear()
        return
    ...
    await message.answer(
        ...
        f"Теперь при регистрации пользователи должны подписаться на него.",
        parse_mode="Markdown"
    )
    await state.clear()
...
async def adm_ad_delete_start(call: types.CallbackQuery):
    ...
    channels = await get_ad_channels()
    if not channels:
        await call.answer("Нет добавленных каналов.", show_alert=True)
        return
    kb = InlineKeyboardBuilder()
    for ch in channels:
        title = ch['title'] or ch['url']
        kb.button(text=f"🗑 {title}", callback_data=f"adm_ad_del_{ch['id']}")
    await call.message.answer(
...
@dp.callback_query(F.data.startswith("adm_ad_del_"))
...
async def adm_reps_cb(call: types.CallbackQuery):
    ...
    last_msg, created_at = r['last_message'], r['created_at']
...
async def r_ban_cb(call: types.CallbackQuery):
    ...
    await db_exec("UPDATE users SET banned_until=$1 WHERE id=$2", (until, target_id))
    if rep_id > 0: await db_exec("DELETE FROM reports WHERE id=$1", (rep_id,))
...
async def r_mute_cb(call: types.CallbackQuery):
    ...
    days, target_id, rep_id = int(parts[1]), int(parts[2]), int(parts[3])
    until = datetime.now() + timedelta(days=days)
    await db_exec("UPDATE users SET muted_until=$1 WHERE id=$2", (until, target_id))
    if rep_id > 0: await db_exec("DELETE FROM reports WHERE id=$1", (rep_id,))
...
async def r_warn_cb(call: types.CallbackQuery):
    ...
    if rep_id > 0: await db_exec("DELETE FROM reports WHERE id=$1", (rep_id,))
    await db_exec("UPDATE users SET warns=COALESCE(warns,0)+1 WHERE id=$1", (target_id,))
    warns = await get_warns(target_id)
    if warns >= 3:
        until = datetime.now() + timedelta(days=3)
        await db_exec("UPDATE users SET banned_until=$1, warns=0 WHERE id=$2", (until, target_id))
        if target_id in active_chats:
            pid = active_chats.pop(target_id)
            active_chats.pop(pid, None)
            try: await bot.send_message(pid, "💔 Собеседник покинул чат.", reply_markup=main_kb(pid))
        ...
        try:
            await bot.send_message(target_id,
                f"🚫 *Вы получили 3 предупреждения и автоматически заблокированы на 3 дня!*\n"
...
async def adm_warn_proc(message: types.Message, state: FSMContext):
    ...
    try:
        tid = int(raw)
        row = await db_exec("SELECT id,username FROM users WHERE id=$1", (tid,), "one")
    except ValueError:
        row = await db_exec("SELECT id,username FROM users WHERE username=$1", (raw,), "one")
    if not row:
        await message.answer("❌ Пользователь не найден.")
        await state.clear(); return
    tid, tusername = row['id'], row['username']
    if tid in ADMINS:
...
async def adm_unwarn_proc(message: types.Message, state: FSMContext):
    ...
    try:
        tid = int(raw)
        row = await db_exec("SELECT id,username,warns FROM users WHERE id=$1", (tid,), "one")
    except ValueError:
        row = await db_exec("SELECT id,username,warns FROM users WHERE username=$1", (raw,), "one")
    if not row:
        await message.answer("❌ Пользователь не найден.")
        await state.clear(); return
    tid, tusername, warns = row['id'], row['username'], row['warns'] or 0
    name = f"@{tusername}" if tusername else str(tid)
    if warns <= 0:
        await message.answer(f"ℹ️ У {name} нет варнов.")
        await state.clear(); return
    new_warns = warns - 1
    await db_exec("UPDATE users SET warns=$1 WHERE id=$2", (new_warns, tid))
    try:
...
async def adm_banlist(call: types.CallbackQuery):
    if call.from_user.id not in ADMINS: return
    now = datetime.now()
    banned = await db_exec("SELECT id,username,banned_until FROM users WHERE banned_until > $1 ORDER BY banned_until DESC LIMIT 10", (now,), "all")
    muted  = await db_exec("SELECT id,username,muted_until FROM users WHERE muted_until > $1 ORDER BY muted_until DESC LIMIT 10", (now,), "all")
    warned = await db_exec("SELECT id,username,warns FROM users WHERE warns > 0 ORDER BY warns DESC LIMIT 10", (), "all")

    txt = "📋 *Активные наказания*\n\n🔨 *Забаненные:*\n"
    if banned:
        for r in banned:
            t = r['banned_until'].strftime('%d.%m.%Y')
            txt += f"• `{r['id']}` @{r['username'] or '—'} до {t}\n"
    else:
        txt += "Нет\n"
    txt += "\n🔇 *Замученные:*\n"
    if muted:
        for r in muted:
            t = r['muted_until'].strftime('%d.%m.%Y')
            txt += f"• `{r['id']}` @{r['username'] or '—'} до {t}\n"
    else:
        txt += "Нет\n"
    txt += "\n⚠️ *С варнами:*\n"
...
async def adm_lookup_proc(message: types.Message, state: FSMContext):
    ...
    try:
        tid = int(raw)
        row = await db_exec("SELECT id,username,refs,prem_until,gender,age,banned_until,muted_until,warns FROM users WHERE id=$1", (tid,), "one")
    except ValueError:
        row = await db_exec("SELECT id,username,refs,prem_until,gender,age,banned_until,muted_until,warns FROM users WHERE username=$1", (raw,), "one")
    if not row:
...
async def adm_revoke_prem_proc(message: types.Message, state: FSMContext):
    ...
    try:
        tid = int(raw)
        row = await db_exec("SELECT id,username,prem_until FROM users WHERE id=$1", (tid,), "one")
    except ValueError:
        row = await db_exec("SELECT id,username,prem_until FROM users WHERE username=$1", (raw,), "one")
    if not row:
        await message.answer("❌ Пользователь не найден.")
        await state.clear(); return
    tid, tusername, prem_until = row['id'], row['username'], row['prem_until']
    name = f"@{tusername}" if tusername else str(tid)
...
async def a_stats_cb(call: types.CallbackQuery):
    ...
    prem_row  = await db_exec("SELECT COUNT(*) AS cnt FROM users WHERE prem_until > $1", (now,), "one")
    ban_row   = await db_exec("SELECT COUNT(*) AS cnt FROM users WHERE banned_until > $1", (now,), "one")
    mute_row  = await db_exec("SELECT COUNT(*) AS cnt FROM users WHERE muted_until > $1", (now,), "one")
    warn_row  = await db_exec("SELECT COUNT(*) AS cnt FROM users WHERE warns > 0", fetch="one")
    rep_row   = await db_exec("SELECT COUNT(*) AS cnt FROM reports", fetch="one")
    ch_row    = await db_exec("SELECT COUNT(*) AS cnt FROM ad_channels", fetch="one")
    prem_c  = prem_row['cnt']  if prem_row  else 0
    ban_c   = ban_row['cnt']   if ban_row   else 0
    mute_c  = mute_row['cnt']  if mute_row  else 0
    warn_c  = warn_row['cnt']  if warn_row  else 0
    rep_c   = rep_row['cnt']   if rep_row   else 0
    ch_c    = ch_row['cnt']    if ch_row    else 0
...
async def buy_cb(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        f"💳 Карта: `4874070057830877`\nСумма: *{price}*\n\nПосля оплаты відправте скриншот чека сюди 👇",
        parse_mode="Markdown"
    )
...
# ================================================================
# ОПРЕДЕЛЕНИЕ РЕКЛАМЫ В СООБЩЕНИИ
# ================================================================
import re

AD_PATTERNS = [
    r"t\.me/[a-zA-Z0-9_+]+",          # t.me/username или t.me/+invite
    r"telegram\.me/[a-zA-Z0-9_+]+",
    r"@[a-zA-Z0-9_]{5,}",              # @username (минимум 5 символов чтобы не срабатывало на обычные упоминания)
    r"https?://t\.me",
    r"https?://telegram",
    r"tg://resolve\?domain=",
]
...
def is_ad_message(text: str) -> bool:
    """Проверяет, содержит ли текст рекламные ссылки или юзернеймы каналов."""
    if not text:
        return False
...
async def handle_ad_warn(uid: int, message: types.Message):
    """Выдаёт варн за рекламу и банит при 3 варнах."""
    ...
    if warns >= 3:
        ...
        await message.answer(
            "🚫 *Обнаружена попытка рекламы в чате!*\n\n"
            f"⚠️ Вы получили 3-й варн и *автоматически заблокированы на 3 дня.*\n"
            f"Разблокировка: {until.strftime('%d.%m.%Y %H:%M')}",
        ...
        await message.answer(
            "🚫 *Обнаружена попытка рекламы в чате!*\n\n"
            f"Рекламные ссылки и упоминания каналов запрещены.\n"
            f"⚠️ Вы получаете варн *{warns} из 3*.\n"
            f"При 3 варнах — автоматический бан на 3 дня.",
            parse_mode="Markdown"
        )

# ================================================================
# ПОДЕЛИТЬСЯ ЮЗЕРНЕЙМОМ В ЧАТЕ
# ================================================================
...
async def share_username(message: types.Message):
    ...
    if username:
        text = f"👤 Собеседник поделился своим юзернеймом:\n@{username}"
    else:
        text = f"👤 Собеседник поделился своим ID:\n`{uid}`"
    try:
        await bot.send_message(pid, text, parse_mode="Markdown")
        await message.answer("✅ Твой юзернейм отправлен собеседнику!")
    except:
        await message.answer("❌ Не удалось отправить.")


# ================================================================
# ОБРАБОТКА ФОТО ДЛЯ ОПЛАТЫ (wait_proof)
# ================================================================
...
# ================================================================
# КНОПКИ ГЛАВНОГО МЕНЮ (работают при любом стейте)
# ================================================================
@dp.message(StateFilter("*"), F.text == "👤 Профиль")
async def menu_profile(message: types.Message, state: FSMContext):
    uid = message.from_user.id
    if uid in active_chats:
        return  # в чате — не реагируем
    await state.clear()
    banned, until = await is_banned(uid)
    if banned:
        await message.answer(f"🚫 Вы заблокированы до *{until}*.", parse_mode="Markdown"); return
    u = await db_exec("SELECT id,username,refs,prem_until,gender,age,banned_until,muted_until,warns FROM users WHERE id=$1", (uid,), "one")
    if not u:
...
# ================================================================
# ГЛОБАЛЬНЫЙ ОБРАБОТЧИК (чат между пользователями)
# ================================================================
